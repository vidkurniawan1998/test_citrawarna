        ...
        <div class="row">
            <div class="col-lg-">
                ...
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title"><i class="bi bi-file-spreadsheet-fill"></i>
                            ...
                            <button class="btn btn-primary btn-sm" style="float: right;" data-toggle="modal"
                                data-target="#modal-create"><i class="bi bi-plus-circle"></i> Barang Masuk</button>
                        </h5>
                        <hr>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody id="myTable">
                                @if (count($barangMasuk) > 0)
                                    @foreach ($barangMasuk as $index => $li)
                                        <tr>
                                ...
                                @else
                                    <tr>
                                        <td colspan="7" style="text-align:center;"><b>Data Barang Masuk Tidak
                                                Ditemukan</b></td>
                                    </tr>
                                @endif
                            </tbody>
                    ...
                    <div class="px-sm-3" style="padding-left:15pt;">Jumlah Data : {{ $barangMasuk->count() }} Dari
                        {{ $barangMasuk->total() }} </div>
                    <div style="padding-left:15pt;">{{ $barangMasuk->links() }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('barangmasuk.create') }}" method="post" enctype="multipart/form-data">
                            @csrf
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-primary update-barangmasuk">Save</button>
                                <input type="button" class="btn btn-secondary" data-dismiss="modal" value="Close">
                            </div>
                        </form>
                    </div>
...
@push('barangmasuk')
    <script>
        $(function() {
            ...
            $('.edit-barangmasuk').click(function() {
                var id = $(this).data('id');
                var url = '{{ url('barangmasuk/edit') }}' + '/' + id;
                $.ajax({
                    url: url,
                    type: 'get',
                    datatype: 'json',
            ...
            $('.update-barangmasuk').click(function() {
                var id = $('.id').val();
                var nama_barang = $('.nama_barang').val();
                var deskripsi = $('.deskripsi').val();
                var qty = $('.qty').val();
                var harga_barang = $('.harga_barang').val();
                var url = '{{ url('barangmasuk/update') }}' + '/' + id;
